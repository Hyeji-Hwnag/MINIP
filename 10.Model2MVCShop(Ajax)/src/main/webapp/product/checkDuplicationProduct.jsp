...
/* window.onload = function(){
	...
	document.getElementById("prodName").onkeydown = function(){
...
/* function fncCheckDuplication() {
	// Form 유효성 검증
	if(document.detailForm.prodName.value != null && document.detailForm.prodName.value.length >0) {
	    document.detailForm.action='/product/checkProductDuplication';
...
$(function () {
	...
	$("td.ct_btn:contains('중복확인')").on("click" , function() {
		...
		if( $("#prodName").val() != null && $("#prodName").val().length >0) {
		    ...
		    $("form").attr("action" , "/product/checkProductDuplication");
		    $("form").submit();
		}else {
			alert('상품명은 반드시 입력하셔야 합니다.');
		}
...
function fncProdName() {
	if(opener) {
		<%-- /////////////////////// EL / JSTL 적용으로 주석 처리 ////////////////////////
		opener.document.detailForm.userId.value = "<%=userId%>";
		 		/////////////////////// EL / JSTL 적용으로 주석 처리 //////////////////////// --%>
		opener.document.detailForm.prodName.value = "${prodName}";
	}
	window.close();
}
$(function() {
	...
	<tr>
		...
		<td background="/images/ct_ttl_img02.gif" width="100%" style="padding-left:10px;">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td width="93%" class="ct_ttl01">상품명 중복확인</td>
				...
				<%	if(result){ %>
					...
					<c:if test="${ ! empty result && result }">
						...
						<td background="/images/ct_btnbg02.gif" class="ct_btn01" style="padding-top:3px;">
							<!-- <a href="javascript:fncProdName();">사용</a> -->
							사용
						</td>
